...
def create_model(data_model):
    ...
    if data_model["grid"]["active"]:
        ...
        def PpvG_lim_rule(m,t):
            return sum(m.ConH['n_dcac',tch]*m.PpvG[tch,t] for tch in m.CH) + m.PTG[t] <= m.GridAv[t]*m.MaxPpvG
    ...
    model.GenOn = Var(model.T, domain=Binary)  
    ...
    if data_model["generator"]["active"]:
        ...
        def PD_lim_rule1(m,t):#
            return m.PDL[t] + sum(m.PDB[tch,tb,t] for tch in model.CH for tb in m.BATT) <= m.GenOn[t]*m.GenPmax
        model.PD_lim1=Constraint(model.T,rule=PD_lim_rule1)

        def PD_lim_rule2(m,t):#
            return m.PDL[t] + sum(m.PDB[tch,tb,t] for tch in model.CH for tb in m.BATT) >= m.GenOn[t]*m.GenMinLoad*m.GenPmax
    ...
    def PV_num_rule3(m,tpv,tch):
        return m.Xpv[tpv,tch]*m.PVtype['P_stc',tpv]/1000 <= sum(m.XCh[tpv,tb,tch] for tb in m.BATT)*m.ConH['P_max_pv',tch]
    model.PV_num_rule3=Constraint(model.PVT,model.CH, rule=PV_num_rule3)
    #----------------------------------------------------------------------#
    ...
    def ComputeSecondStageCost(m):
        return VPN_FS*sum(m.Price_Grid[t]*(m.PGL[t] + sum(m.PGB[tch,tb,t] for tb in m.BATT for tch in m.CH))  +
                        m.FuelCost*(m.GenFmin*m.GenOn[t] + m.GenFm*(m.PDL[t] + sum(m.PDB[tch,tb,t] for tch in model.CH for tb in m.BATT))) + m.GenOMCost*m.GenOn[t] +
